...
#----- CREATE -----
# persistence function to register a real vehicle
def register_vehicle_dao(license_plate: str, 
                            access_key_hash: str, 
                            owner_name: str):
...
#----- READ -----
# persistence function to get all real (active) vehicles
def get_all_real_vehicles_dao():
    sql = """SELECT driver_id,license_plate 
             FROM vehicle 
             WHERE vehicle_type = %s AND status = %s
             ORDER BY driver_id"""

    with get_connection() as conn:
        with conn.cursor() as cur: 
            cur.execute(sql, ('REAL', 'ACTIVE'))
            record = cur.fetchall()
...
# persistence function to login with an existing real (active) vehicle
def vehicle_login_dao(license_plate: int):
...
def suspend_vehicle_dao(license_plate: int):
    sql = """UPDATE vehicle
             SET status = %s 
